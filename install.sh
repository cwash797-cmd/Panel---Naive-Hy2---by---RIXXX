...
if [[ "$MASQUERADE_MODE_INPUT" == "2" ]]; then
  ...
  echo ""
  read -rp "  URL для зеркалирования (например https://www.iana.org): " MASQUERADE_URL
  # Простая валидация: должно начинаться с http:// или https://
  if [[ ! "$MASQUERADE_URL" =~ ^https?:// ]]; then
    log_warn "URL должен начинаться с http:// или https://. Использую дефолт https://www.iana.org"
